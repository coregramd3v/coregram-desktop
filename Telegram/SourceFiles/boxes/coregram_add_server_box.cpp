...
: RpWidget(parent) {
	...
	_name = Ui::CreateChild<Ui::InputField>(
		...
		tr::lng_coregram_server_name());
	_desc = Ui::CreateChild<Ui::InputField>(
		this,
		st::defaultInputField,
		tr::lng_coregram_server_description());
}
...
HostPortRow::HostPortRow(QWidget *parent) : RpWidget(parent) {
	_host = Ui::CreateChild<Ui::InputField>(
		this,
		st::defaultInputField,
		tr::lng_coregram_server_host_hint());
	_port = Ui::CreateChild<Ui::InputField>(
		this,
		st::defaultInputField,
		tr::lng_coregram_server_port());
}
...
AddServerBox::AddServerBox(
	QWidget*,
	Fn<void(CoreGram::Server)> done,
	CoreGram::Server existing)
: _done(std::move(done))
, _content(this)
, _typeGroup(std::make_shared<Ui::RadiobuttonGroup>(0)) {
	...
	_content->add(
		object_ptr<Ui::FlatLabel>(
			_content,
			tr::lng_coregram_server_rsa_key(tr::now),
			st::boxLabel),
		st::boxRowPadding);
	_rsaPublicKey = _content->add(
		object_ptr<Ui::InputField>(
			...
			tr::lng_coregram_server_rsa_key_hint()),
		st::boxRowPadding);
...
void AddServerBox::prepare() {
	setTitle(_editingId.isEmpty()
		? tr::lng_coregram_server_add_title()
		: tr::lng_coregram_server_edit_title());
...
void AddServerBox::chooseLogo() {
	const auto callback = [=](FileDialog::OpenResult &&result) {
		...
		if (image.isNull()) {
			Ui::Toast::Show(tr::lng_coregram_server_logo_invalid(tr::now));
			return;
		}
	...
	FileDialog::GetOpenPath(
		this,
		tr::lng_coregram_server_logo_choose(tr::now),
...
void AddServerBox::save() {
	...
	if (!CoreGram::IsValidRsaPublicKeyPem(rsaPublicKey)) {
		Ui::Toast::Show(tr::lng_coregram_server_rsa_key_invalid(tr::now));
		_rsaPublicKey->setFocusFast();
		return;
	}

	if (!_editingId.isEmpty()) {
		CoreGram::RemoveCustomServer(_editingId);
	}
	if (const auto server = CoreGram::AddCustomServer(
			name,
			host,
			port,
			description,
