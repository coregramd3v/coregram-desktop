...
, _title(
	this,
	tr::lng_coregram_server_details_description(tr::now),
...
, _name(this, name, st::introServerCardName) {
	...
	_logo->paintRequest(
	) | rpl::on_next([=, path = CoreGram::ResolveServerLogoPath(logoPath)] {
...
: RpWidget(parent)
, _status(this, tr::lng_coregram_server_checking(tr::now), st::introServerCardStatusOnline)
...
void StatusBlock::setOnline(bool online, int latencyMs) {
	...
	_latency->setText(online && latencyMs >= 0
		? tr::lng_coregram_server_latency(
			tr::now,
			lt_latency,
...
, _content(this) {
	...
	_content->add(
		object_ptr<DetailRow>(
			_content,
			tr::lng_coregram_server_details_host(tr::now),
			_server.host),
		rowMargin);
	_content->add(
		object_ptr<DetailRow>(
			_content,
			tr::lng_coregram_server_details_port(tr::now),
	...
	_content->add(
		object_ptr<Ui::FlatLabel>(
			_content,
			tr::lng_coregram_server_details_status(tr::now),
			st::introServerDetailsSectionTitle),
		sectionMargin);
...
void ServerDetailsBox::prepare() {
	setTitle(tr::lng_coregram_server_details_title());
	if (_connect) {
		if (CoreGram::IsRemovableServer(_server)) {
			const auto weak = base::make_weak(this);
			addLeftButton(tr::lng_coregram_server_delete(), [=] {
				getDelegate()->show(Ui::MakeConfirmBox({
					.text = tr::lng_coregram_server_delete_confirm(
						tr::now,
						lt_name,
						_server.name),
					.confirmed = crl::guard(weak, [=](Fn<void()> close) {
						...
						if (CoreGram::RemoveCustomServer(_server.id)) {
							if (_removed) {
								_removed();
							}
							closeBox();
					...
					.confirmText = tr::lng_coregram_server_delete(),
					.confirmStyle = &st::attentionBoxButton,
				}));
			}, st::attentionBoxButton);
		}
		addButton(tr::lng_coregram_server_join(), [=] {
			_connect(_server);
			closeBox();
		});
		addButton(tr::lng_cancel(), [=] { closeBox(); });
		if (_edit && CoreGram::IsRemovableServer(_server)) {
