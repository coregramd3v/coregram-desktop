...
, _endpoint(
	this,
	tr::lng_coregram_server_endpoint_short(
		tr::now,
		lt_host,
		_server.host,
		lt_port,
...
, _status(this, tr::lng_coregram_server_checking(tr::now), st::introServerRowStatus)
, _latency(this, QString(), st::introServerRowLatency)
, _joinButton(
	this,
	rpl::single(tr::lng_coregram_server_join(tr::now)),
	...
	_logo->paintRequest(
	) | rpl::on_next([=, path = CoreGram::ResolveServerLogoPath(_server.logoPath)] {
	...
	CoreGram::CheckServerOnline(_server, crl::guard(this, [=](
			bool online,
			int latencyMs) {
...
void ServerRow::setStatus(bool online, int latencyMs) {
	...
	_status->setText(online
		? tr::lng_coregram_server_online(tr::now)
		: tr::lng_coregram_server_offline(tr::now));
	_status->setTextColorOverride(online
		? st::windowActiveTextFg->c
		: st::attentionButtonFg->c);
	if (online && latencyMs >= 0) {
		_latency->setText(tr::lng_coregram_server_latency_short(
			tr::now,
			lt_latency,
...
, _ownTitle(
	Ui::CreateChild<Ui::FlatLabel>(
		this,
		tr::lng_coregram_server_selection_title(tr::now),
		st::introTitle))
, _ownDescription(
	Ui::CreateChild<Ui::FlatLabel>(
		this,
		tr::lng_coregram_server_selection_subtitle(tr::now),
		st::introDescription))
, _addServer(
	this,
	rpl::single(tr::lng_coregram_server_add(tr::now)),
...
, _statusTimer([=] { rebuildList(); }) {
	...
	_addServer->setClickedCallback([=] {
		...
		Ui::show(Box<AddServerBox>([=](CoreGram::Server server) {
...
bool ServerSelectWidget::hasBack() const {
	return Core::App().domain().maybeLastOrSomeAuthedAccount() != nullptr;
}
...
void ServerSelectWidget::rebuildList() {
	...
	const auto join = [=](const CoreGram::Server &server) {
		joinServer(server);
	};
	const auto details = [=](const CoreGram::Server &server) {
		const auto weak = base::make_weak(this);
		const auto editFn = CoreGram::IsRemovableServer(server)
	...
	auto servers = std::vector<CoreGram::Server>();
...
void ServerSelectWidget::joinServer(const CoreGram::Server &server) {
	if (!server.valid()) {
		Ui::Toast::Show(tr::lng_coregram_server_invalid(tr::now));
		return;
	}
	...
	const auto weak = base::make_weak(this);
	CoreGram::CheckServerOnline(server, crl::guard(weak, [=](bool online, int) {
		if (!weak) {
			return;
		}
		if (!online) {
			Ui::show(Ui::MakeConfirmBox({
				.text = tr::lng_coregram_server_offline_confirm(),
				.confirmed = crl::guard(weak, [=] {
					proceedJoin(server);
				}),
				.confirmText = tr::lng_coregram_server_join(),
			}));
			return;
		}
		proceedJoin(server);
...
void ServerSelectWidget::proceedJoin(const CoreGram::Server &server) {
	if (!server.valid()) {
		Ui::Toast::Show(tr::lng_coregram_server_invalid(tr::now));
		return;
	}
	...
	CoreGram::WaitForServerConnection(&account(), server, crl::guard(weak, [=](
			bool ok) {
		if (!weak) {
			return;
		}
