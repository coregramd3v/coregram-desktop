...
void AvatarCornersPreview::paintEvent(QPaintEvent *e) {
	...
	const auto nameText = u"EternalGram Releases"_q;
...
void AvatarCornersPreview::mouseReleaseEvent(QMouseEvent *e) {
	...
	if (e->button() == Qt::LeftButton && rect().contains(e->pos())) {
		_controller->showPeerByLink(Window::PeerByLinkInfo{
			.usernameOrId = u"EternalGramReleases"_q,
		});
	}
}
...
void AvatarCornersPreview::resolveChannel() {
	...
	_peer = session->data().peerByUsername(u"EternalGramReleases"_q);
	if (_peer) {
		_peer->loadUserpic();
		subscribeToUpdates();
		return;
	...
	session->api().request(MTPcontacts_ResolveUsername(
		MTP_flags(0),
		MTP_string(u"EternalGramReleases"_q),
		MTP_string()
