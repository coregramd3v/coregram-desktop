...
const auto kServersFile = u"coregram_servers.json"_q;
const auto kServerLogosDir = u"coregram_server_logos"_q;
constexpr auto kCheckTimeoutMs = 3000;
constexpr auto kConnectTimeoutMs = 30000;
const auto kOfficialDefaultHost = u"2.26.123.219"_q;
constexpr auto kOfficialDefaultPort = 2398;
constexpr auto kOfficialDefaultDcId = 2;

// Default RSA public key for the built-in self-hosted CoreGram server.
...
[[nodiscard]] std::optional<Storage::CoreServerSelection>
ReadSavedServerSelection(not_null<Main::Account*> account) {
	return account->local().readCoreServer();
}

[[nodiscard]] bool SelectionEqualsServer(
		const Storage::CoreServerSelection &selection,
		const Server &server) {
...
[[nodiscard]] MTP::DcId MainDcIdForServer(const Server &server) {
	...
	// default to the data center id the CoreGram/teamgram backend runs on.
	return MTP::DcId(server.multiDc ? 2 : kOfficialDefaultDcId);
}
...
void ApplyServerToDcOptions(
	...
	if (server.multiDc) {
		...
		if (!server.rsaPublicKey.isEmpty()) {
			...
			dcOptions->setBuiltInPublicKeys();
		}
		return;
	}
...
[[nodiscard]] Server ServerFromStoredSelection(
	...
	if (selection.id == QString::fromLatin1(kTelegramServerId)) {
		...
		// Unknown custom server: a single-server backend by default.
...
Server TelegramServer() {
	...
	result.name = tr::lng_coregram_server_telegram_name(tr::now);
	result.description = tr::lng_coregram_server_telegram_description(tr::now);
...
Server OfficialServer() {
	...
	result.name = tr::lng_coregram_server_official_name(tr::now);
	result.description = tr::lng_coregram_server_official_description(tr::now);
	...
	result.mainDcId = kOfficialDefaultDcId;
	return result;
}
...
void MigrateLegacyServerSelection(
		not_null<Main::Account*> account,
		bool authorized) {
	if (!authorized || ReadSavedServerSelection(account)) {
		return;
	}
	const auto server = OfficialServer();
	account->local().writeCoreServer(server.id, server.host, server.port);
}
...
void RestoreServerToConfig(
	...
	if (!selection) {
		...
		// If the config somehow has locked options (inherited from a CoreGram
		// fallback config), unlock it so the new account doesn't silently talk
		// to CoreGram before the user picks a server.
	...
	if (server.isTelegram) {
		...
		// backend (CoreGram/custom), which force-maps every dc_id
		// 1..5 onto that one non-Telegram host (see the single-server branch
		// of ApplyServerToDcOptions above). In that case DC1/3/4/5 are
		...
		// prior CoreGram login: AuthKey/RSA and empty dc_options errors).
		//
		...
		config->dcOptions().setBuiltInPublicKeys();
		config->dcOptions().setOptionsLocked(false);
		if (wasLocked) {
