...
inline constexpr auto kOfficialServerId = "coregram";
...
struct Server {
	...
	// (CoreGram/teamgram/custom) are one physical machine, so every
	// dc_id is mapped onto the single host:port instead.
	bool multiDc = false;
	// Home data-center id. 0 = auto (multiDc -> 2, single-server -> 2).
	int mainDcId = 0;
...
// Accounts authorized before multi-server support have no stored selection, but
// they were talking to the built-in CoreGram server. Pin that choice once for
// them, so the "no selection = server not chosen yet" rule (which leaves the DC
// options unlocked, i.e. pointing at the built-in Telegram table) never resets a
// working legacy session onto the wrong network. `authorized` must be false for
// accounts that have no stored MTP auth keys — those really are new.
